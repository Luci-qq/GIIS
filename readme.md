...
6. [Лабораторная работа #6 - Заливка многосвязных областей](#lab6)
7. [Лабораторная работа #7 - Триангуляция Делоне и Диаграмма Вороного](#lab7)
...
# Лабораторная работа #6 - Заливка многосвязных областей

Этот код написан на Python и использует библиотеку Tkinter для создания графического пользовательского интерфейса (GUI). Программа позволяет заливать многосвязные области на холсте с использованием различных алгоритмов заливки: простой затравочный, затравочный построчный и растровой развертки. Также имеется возможность загрузить предопределенные точки из файла `points.txt`.

## Алгоритмы заливки

### Простой затравочный алгоритм

Простой затравочный алгоритм использует стек для хранения точек, которые необходимо залить. Алгоритм начинается с выбранной точки и рекурсивно заливает все соседние точки, которые не являются границей области. Он работает следующим образом:

1. Добавить начальную точку в стек.
2. Извлечь точку из стека и залить ее.
3. Проверить соседние точки (верхнюю, нижнюю, левую и правую) и добавить их в стек, если они не граничные и не залиты.
4. Повторять шаги 2 и 3 до тех пор, пока стек не опустеет.

Этот алгоритм прост в реализации, но может быть неэффективным для больших областей из-за чрезмерного использования рекурсии и стека.

### Затравочный построчный алгоритм

Затравочный построчный алгоритм является модификацией простого затравочного алгоритма. Вместо обхода всех соседних точек, он заливает целые строки за один проход. Алгоритм работает следующим образом:

1. Добавить начальную точку в стек.
2. Извлечь точку из стека и залить ее.
3. Для каждой незалитой точки рядом с текущей:

- Залить все незалитые точки в той же строке влево и вправо от этой точки.
- Добавить крайние левую и правую точки следующей строки выше и ниже в стек.

4. Повторять шаги 2 и 3 до тех пор, пока стек не опустеет.

Этот алгоритм более эффективен, чем простой затравочный алгоритм, особенно для больших областей, так как он уменьшает количество операций с соседними точками.

### Алгоритм растровой развертки

Алгоритм растровой развертки не использует стек или рекурсию. Вместо этого он сканирует область по строкам и заливает незалитые точки, которые находятся внутри границ области. Алгоритм работает следующим образом:

1. Отсортировать все граничные точки по возрастанию координаты `y`, а затем по возрастанию координаты `x`.
2. Для каждой координаты `y`:

- Создать список интервалов `x`, которые представляют незалитые области в этой строке.
- Залить все точки внутри этих интервалов.

Этот алгоритм более эффективен, чем затравочные алгоритмы, так как он не использует стек или рекурсию, что позволяет избежать проблем переполнения стека для очень больших областей.

## Структура кода

- Функция `get_point()` читает точки из файла `points.txt` и возвращает их в виде списка списков.
- Функция `create_grem()` отображает граничные точки на холсте и сохраняет их в глобальной переменной `pointsMax`.
- Функция `delay()` вызывается для визуализации процесса заливки, создавая небольшие прямоугольники на залитых точках.
- Функция `zero()` очищает холст.
- Функции `simple_Z()`, `str_Z()` и `razv()` реализуют соответствующие алгоритмы заливки и привязываются к нажатию правой кнопки мыши на холсте с помощью функции `bind_method()`.
- Функция `clear_canvas()` очищает холст.
- Функция `reset_all()` очищает холст и сбрасывает граничные точки.
- Создается графический интерфейс с холстом, кнопкой очистки и меню для выбора алгоритма загрузки граничных точек и метода заливки.

## Использование программы

1. Запустите скрипт в Python.
2. В открывшемся окне выберите алгоритм загрузки граничных точек из меню "Алгоритм" (например, "Грэхам").
3. Выберите алгоритм заливки из меню "Метод" (например, "Простой затравочный", "Затравочный построчный" или "Растровой развертки").
4. Нажмите правую кнопку мыши на холсте, чтобы начать заливку области, содержащей курсор мыши.
5. Чтобы очистить холст, нажмите кнопку "Очистить холст".
...
Этот код представляет собой реализацию двух алгоритмов: Триангуляции Делоне и Диаграммы Вороного. Он использует библиотеки Tkinter для создания графического интерфейса и SciPy для расчетов.

## Функциональность

- Пользователь может добавлять точки на холст, нажимая левую кнопку мыши.
- При добавлении трех и более точек строится Триангуляция Делоне, соединяющая точки в треугольники.
- Нажатие правой кнопки мыши при выборе опции "Триангуляция Делоне" добавляет новую точку и обновляет триангуляцию.
- Нажатие правой кнопки мыши при выборе опции "Диаграмма Вороного" строит диаграмму Вороного для всех добавленных точек.
- Меню "Алгоритм" позволяет очистить холст и начать заново.
- Меню "Метод" позволяет выбрать между Триангуляцией Делоне и Диаграммой Вороного.

## Методы

### Триангуляция Делоне

Триангуляция Делоне - это способ разбиения множества точек на плоскости на непересекающиеся треугольники таким образом, что внутренняя область окружности, описанной вокруг любого треугольника, не содержит других точек множества. Другими словами, триангуляция Делоне максимизирует минимальный угол во всех треугольниках. Это делает её полезной в областях, где требуется создание надежных триангуляционных сеток, таких как компьютерная графика, обработка изображений и моделирование поверхностей.

### Диаграмма Вороного

Диаграмма Вороного - это разбиение плоскости на выпуклые многоугольники вокруг заданного набора точек. Каждый многоугольник состоит из всех точек плоскости, которые ближе к соответствующей точке, чем к любой другой точке из заданного набора. Диаграммы Вороного используются во многих областях, таких как робототехника, компьютерная анимация, метеорология, эпидемиология и операционные исследования.

## Использование

1. Запустите скрипт.
2. Выберите желаемый метод в меню "Метод".
3. Добавляйте точки на холст, нажимая левую кнопку мыши.
4. Для Триангуляции Делоне добавляйте новые точки, нажимая правую кнопку мыши.
5. Для Диаграммы Вороного нажмите правую кнопку мыши после добавления всех точек.
6. Используйте меню "Алгоритм" для очистки холста и начала заново.
...
Примечание: Этот код является демонстрацией и может быть дополнен или изменен для достижения более сложных задач.
