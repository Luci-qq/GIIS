...
x = h + a _ cos(t)
y = k + b _ sin(t), где `a` и `b` - длины полуосей, `h` и `k` - координаты центра, а `t` принимает значения от 0 до 2π.

### Парабола

Для построения параболы используется уравнение:
y = ax^2, где `a` - коэффициент параболы.

### Гипербола

Для построения гиперболы используется уравнение:
(x - h)^2 / a^2 - (y - k)^2 / b^2 = 1, где `a` и `b` - длины полуосей, `h` и `k` - координаты центра.
